# Project Genesis <img src="https://cdn.warframestat.us/genesis/img/avatar.png" height="25" width="25" alt="Genesis Avatar"/>
...
### A [Discord.js](http://discord.js.org) bot for tracking Warframe alerts, invasions, and much, much more.
...
[![Discord Bots](https://discordbots.org/api/widget/123591822579597315.png)](https://discordbots.org/bot/123591822579597315?utm_source=widget)
...
1. Start bot with [pm2](http://pm2.keymetrics.io/) using a copy of the provided `pm2.json` file. The scripts assume it's named `genesis.json` (`npm start`, `npm run start:logs`)
...
CLUSTER_ID | Cluster identifier (0-indexed) | 4 | 0
...
RAVEN_URL | DSN url for logging errors to Sentry | `'https://***:***@sentry.io/***'` | N\A
INVITE_URL | Sends the user an OAuth or other URL for inviting the bot | `https://discord.gg/discord-api` | `'No Invite Set'`
...
Script | Purpose
--- | ---
`lint` | Lint the bot files, show a list of style or syntax issues
`lint:fix` | Fix any automatically fixable issues, display others
`logs` | show the bot logs from pm2
`test` | run tests.... this literally just calls lint
`restart` | delete the bot (genesis) pm2 process and restart it with logs immediately
`setup` | runs install
`start` | starts the bot through pm2
`start:logs` | starts the bot through pm2 & shows logs
`stop` | deletes the bot (genesis) pm2 process
`uninstall` | kills pm2, deletes node modules for genesis, uninstalls pm2 ***(THIS IS SUPER DESTRUCTIVE)***
`update:genesis` | runs the loosest process to update the bot. deletes the bot process, clears the local workspace, pulls changes for the current branch, installs dependencies, and either restarts or start the bot
